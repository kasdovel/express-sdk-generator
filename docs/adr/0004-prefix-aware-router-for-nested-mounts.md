...
router and knows its mount prefix, so `.route(...)` registers the *local* path on Express
(mounting composes normally) while recording the *full* accumulated path in the registry.
...
`router`. We chose **router**: it is the truthful name (the thing *is* an Express router plus
